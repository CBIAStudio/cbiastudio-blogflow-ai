...
ok_case(source_has($openai, 'for ($t = 1; $t <= 1; $t++)'), 'maximum two attempts total');
...
ok_case(source_has($posts, 'already_sufficient'), 'initial article sufficient');
ok_case(source_has($posts, 'Length below target'), 'initial article short');
ok_case(source_has($posts, "? 'sufficient' : 'still_short'"), 'expansion sufficient');
ok_case(source_has($posts, "'still_short'"), 'expansion insufficient');
ok_case(source_has($posts, 'cbia_openai_expansion_input_token_guard'), 'expansion excessive input');
