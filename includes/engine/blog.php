    ...
    function cbia_create_all_posts_checkpointed($incoming_titles=null, $max_per_run = 1){
        ...
        foreach ($queue as $i => $title) {
			...
			if (!empty($result['efficiency']) && is_array($result['efficiency'])) {
				$efficiency_defaults = array('posts'=>0,'first_pass_success'=>0,'first_pass_nominal_target_met'=>0,'accepted_with_tolerance'=>0,'expansions'=>0,'incremental_expansions'=>0,'full_expansions'=>0,'failed_length'=>0,'first_words'=>0,'final_words'=>0,'reasoning_tokens'=>0,'token_limits'=>0,'initial_cost_micro'=>0,'expansion_cost_micro'=>0,'total_cost_micro'=>0,'thinking_modes'=>array());
				$cp['efficiency'] = array_merge($efficiency_defaults, is_array($cp['efficiency'] ?? null) ? $cp['efficiency'] : array());
				...
				$cp['efficiency']['expansions'] += (int)$metric['expansion_used']; $cp['efficiency']['first_pass_success'] += !empty($metric['first_pass_accepted']) ? 1 : 0; $cp['efficiency']['first_pass_nominal_target_met'] += !empty($metric['first_pass_nominal_target_met']) ? 1 : 0; $cp['efficiency']['accepted_with_tolerance'] += !empty($metric['accepted_with_tolerance']) ? 1 : 0; $cp['efficiency']['reasoning_tokens'] += (int)$metric['reasoning_tokens']; $cp['efficiency']['token_limits'] += (int)$metric['hit_token_limit'];
				if (!empty($metric['expansion_used']) && ($metric['expansion_type'] ?? '') === 'incremental') $cp['efficiency']['incremental_expansions']++; elseif (!empty($metric['expansion_used'])) $cp['efficiency']['full_expansions']++;
				$cp['efficiency']['initial_cost_micro'] += (int)($metric['initial_request_cost_micro_usd'] ?? 0); $cp['efficiency']['expansion_cost_micro'] += (int)($metric['expansion_cost_micro_usd'] ?? 0); $cp['efficiency']['total_cost_micro'] += (int)($metric['total_text_cost_micro_usd'] ?? 0);
        ...
        if ($queue_count > 0 && $idx_now >= $queue_count) {
			...
			$expansion_rate = (int)round(((int)($eff['expansions'] ?? 0) * 100) / $eff_posts);
			cbia_log_message('[INFO] Text generation efficiency: posts=' . (int)($eff['posts'] ?? 0) . ' first_pass_accepted=' . (int)($eff['first_pass_success'] ?? 0) . ' first_pass_nominal_target_met=' . (int)($eff['first_pass_nominal_target_met'] ?? 0) . ' accepted_with_tolerance=' . (int)($eff['accepted_with_tolerance'] ?? 0) . ' expansions=' . (int)($eff['expansions'] ?? 0) . ' expansion_rate=' . $expansion_rate . '% average_first_pass_words=' . (int)round(((int)($eff['first_words'] ?? 0)) / $eff_posts) . ' average_final_words=' . (int)round(((int)($eff['final_words'] ?? 0)) / $eff_posts) . ' initial_cost_total=$' . number_format(((int)($eff['initial_cost_micro'] ?? 0)) / 1000000, 6, '.', '') . ' expansion_cost_total=$' . number_format(((int)($eff['expansion_cost_micro'] ?? 0)) / 1000000, 6, '.', '') . ' total_text_cost=$' . number_format(((int)($eff['total_cost_micro'] ?? 0)) / 1000000, 6, '.', '') . ' incremental_expansions=' . (int)($eff['incremental_expansions'] ?? 0) . ' full_expansions=' . (int)($eff['full_expansions'] ?? 0) . ' responses_hitting_token_limit=' . (int)($eff['token_limits'] ?? 0) . '.');
