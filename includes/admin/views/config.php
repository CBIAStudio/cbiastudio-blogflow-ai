...
if (!isset($s['post_language'])) $s['post_language'] = 'Spanish';
...
// Free edition: no internal images.
...
if ($saved_flag !== '') {
    echo '<div class="notice notice-success is-dismissible" style="background: rgba(34, 211, 238, 0.1); border-color: var(--abb-cyan); color: var(--abb-cyan);"><p>' . esc_html__('Configuration saved successfully.', 'cbiastudio-blogflow-ai') . '</p></div>';
}
...
echo '<p class="description">' . esc_html__('Configure AI engines for text and images.', 'cbiastudio-blogflow-ai') . '</p>';
echo '</div>';
...
foreach ($providers_list as $pkey => $pdef) {
    ...
    foreach ($text_list as $mdl) {
        ...
        if ($pkey === 'openai' && $mdl === $recommended) $label .= ' (Recommended)';
...
foreach ($image_providers_list as $pkey => $pdef) {
    ...
    if (empty($img_list)) {
        ...
        foreach ($img_list as $mdl) {
            $label = $mdl;
            if ($mdl === 'imagen-3.0-generate-002') $label .= ' (Recommended)';
...
echo '<p class="description">' . esc_html__('Increase this value if content is cut off. Recommended: 6000-8000.', 'cbiastudio-blogflow-ai') . '</p>';
echo '</td></tr>';

echo '<tr><th scope="row"><label>' . esc_html__('Post length', 'cbiastudio-blogflow-ai') . '</label></th><td>';
$variants = [
    'short'  => 'Short (~1000 words)',
    'medium' => 'Medium (~1800-2000 words)',
    'long'   => 'Long (~2000-2200 words)',
];
...
echo '<p class="description">' . esc_html__('The base prompt is already optimized. It is automatically combined with [IMAGE: ...] and the post title. Adjust only if you want a different style.', 'cbiastudio-blogflow-ai') . '</p>';
...
echo '</div>';
echo '<p class="description">' . esc_html__('The post title is automatically inserted into {title}.', 'cbiastudio-blogflow-ai') . '</p>';
echo '</div>';
...
// Free edition: no internal images (featured image only). UI removed.
...
echo '</select>';
echo '<p class="description">' . esc_html__('By default, the next model is tried on error.', 'cbiastudio-blogflow-ai') . '</p>';
echo '</td></tr>';
...
if (!$yoast_active) {
    echo '<p class="description"><strong>Yoast SEO:</strong> not detected. If installed, the plugin can automatically update meta description, keyphrase, and SEO/readability scores when creating each post.</p>';
} else {
    echo '<p class="description"><strong>Yoast SEO:</strong> detected. Meta description, keyphrase, and SEO/readability scores are automatically updated when creating each post.</p>';
}
echo '</div>';

echo '<p>';
echo '<button type="submit" name="cbia_config_save" class="button button-primary">' . esc_html__('Save configuration', 'cbiastudio-blogflow-ai') . '</button>';
echo '</p>';
